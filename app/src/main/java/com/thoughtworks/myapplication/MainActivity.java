...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        averageTextView = (TextView) findViewById(R.id.text_view_average);
        ...
        if (data != null && !data.isEmpty()) {
            ...
            for(int i=0; i<length; i++) {
                ...
                if (pm25.getQuality() == null) {
                    pm25.setQuality("Undetected");
                    pm25.setAqi("");
                }
                info += "Detect Area: " + pm25.getPositionName()
                        + " " + pm25.getQuality()
                        + " " + pm25.getAqi()
                        + "\n";
            }
            pm25TextView.setText(info);

            String average = "";
            PM25 pm25 = data.get(length);
            average += "\nAverage Quality: " + pm25.getQuality()  + " " + pm25.getAqi() + "\n";
            changeTextViewColor(Integer.parseInt(pm25.getAqi()));
            averageTextView.setText(average);
        }
    }

    private void changeTextViewColor(int aqi)
    {
        if(aqi <= 50)
            averageTextView.setTextColor(Color.rgb(0,255,0));
        else if(aqi <= 200 && aqi > 50)
            averageTextView.setTextColor(Color.rgb(0,0,255));
        else
            averageTextView.setTextColor(Color.rgb(255,0,0));
    }
}
